...
def agregar_medicamento():
    #Solicita los datos de un medicamento, los valida y devuelve un diccionario con su información.
...
def pedir_fecha(mensaje):
    #Solicita una fecha hasta que tenga un formato válido y la convierte en un objeto datetime.
    fecha = input(mensaje)
...
def mostrar_medicamentos(medicamentos):
    #Muestra en pantalla la información de todos los medicamentos registrados.
    if not medicamentos:
...
def registrar_toma(medicamentos, nombre_buscar):
    #Busca un medicamento, registra una toma y descuenta la dosis correspondiente de su stock.
    for med in medicamentos:
...
def verificar_alertas(medicamentos, umbral_stock, dias_vencimiento):
    #Revisa el stock y el vencimiento de los medicamentos para informar posibles alertas.
    hoy = datetime.now()
    alerta_stock = False
...
def calcular_dias_restantes(medicamentos):
    #Calcula aproximadamente cuántos días durará el stock de cada medicamento según su consumo diario.
    for med in medicamentos:
        if med['dosis'] > 0:
...
def buscar_medicamento(medicamentos, nombre_buscar):
    #Busca un medicamento por su nombre y muestra todos sus datos si lo encuentra.
    for med in medicamentos:
        if med['nombre'].lower() == nombre_buscar.lower():
            print(f"\nNombre: {med['nombre']}")
...
def hablar_con_ia():
    #Inicia un chat con un asistente de IA especializado en consultas relacionadas con medicamentos.
    historial = [
        {
            "role": "system",
